...
# This lightweight FastAPI application provides the HTTP entrypoint while
# the Telegram bot remains available when this file is run directly.
from fastapi import FastAPI

app = FastAPI(title="RED LUCKY XYZ Hosting Bot")

@app.get("/")
def health_check():
    return {"status": "ok", "service": "RED LUCKY XYZ Hosting Bot"}
...
# -------------------- CONFIGURATION --------------------
BOT_TOKEN = "8805059380:AAETAMZ7Q8omqJPvJ2yfnsDymEOEJcXqzow"   # Enter your token
...
# ---------- REMOVE WEBHOOK TO AVOID CONFLICT ----------
try:
    bot.remove_webhook()
    time.sleep(0.5)
except:
...
def start_bot():
    logger.info("🌌 RED LUCKY XYZ BOT (Conflict Fix) is starting...")
    while True:
        try:
...
if __name__ == "__main__":
    # Uncomment the line below to run in the background
    # run_daemon()
    start_bot()